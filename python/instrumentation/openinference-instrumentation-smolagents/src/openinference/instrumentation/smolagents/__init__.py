...
class SmolagentsInstrumentor(BaseInstrumentor):  # type: ignore
    ...
    def _instrument(self, **kwargs: Any) -> None:
        ...
        for model_class in exported_model_classes:
            model_wrapper = _ModelWrapper(tracer=self._tracer)
            self._original_model_call_methods[model_class] = getattr(model_class, "__call__")
            wrap_function_wrapper(
                module="smolagents",
                name=f"{model_class.__name__}.__call__",
